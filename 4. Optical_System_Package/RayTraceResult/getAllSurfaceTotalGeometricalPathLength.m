function [ totalPathLengths ] = getAllSurfaceTotalGeometricalPathLength( allSurfaceRayTraceResult,...
        rayPupilIndices,rayFieldIndices,rayWavelengthIndices)
    %getAllSurfaceTotalPathLength: Returns the Total Geometrical Path Length of a specific
    ...
    %   ray
    % Output:
    %   totalPathLengths: is (1 X nSurface X nPupilPointsRequested X nFieldRequested X nWavRequested)
    
    if nargin == 0
        disp(['Error: The function  getAllSurfaceTotalPathLength requires ',...
            'atleast the surface trace result struct as argument.']);
        totalPathLengths = NaN;
        return;
    elseif nargin == 1
    ...
    requestedResultFieldName = 'TotalGeometricalPathLength';
    requestedFieldFirstDim = 1;
    totalPathLengths = getRayTraceResultFieldForAllSurfaces( ...
