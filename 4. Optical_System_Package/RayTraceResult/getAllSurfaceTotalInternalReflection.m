function [ totalInternalReflections ] = getAllSurfaceTotalInternalReflection( allSurfaceRayTraceResult,...
    ...
    %   ray
    % Output:
    %   totalInternalReflections: is (1 X nSurface X nPupilPointsRequested X nFieldRequested X nWavRequested)
    
    if nargin == 0
        disp(['Error: The function  getAllSurfaceTotalInternalReflection requires ',...
            'atleast the surface trace result struct as argument.']);
        totalInternalReflections = NaN;
        return;
    elseif nargin == 1
    ...
    requestedResultFieldName = 'TotalInternalReflection';
    requestedFieldFirstDim = 1;
    totalInternalReflections = getRayTraceResultFieldForAllSurfaces( ...
