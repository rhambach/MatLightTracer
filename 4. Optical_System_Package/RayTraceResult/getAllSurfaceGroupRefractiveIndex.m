function [ groupRefractiveIndexs ] = getAllSurfaceGroupRefractiveIndex( allSurfaceRayTraceResult,...
    ...
    %   ray
    % Output:
    %   groupRefractiveIndexs: is (1 X nSurface X nPupilPointsRequested X nFieldRequested X nWavRequested)
    
    if nargin == 0
        disp(['Error: The function  getAllSurfaceGroupRefractiveIndex requires ',...
            'atleast the surface trace result struct as argument.']);
        groupRefractiveIndexs = NaN;
        return;
    elseif nargin == 1
    ...
    requestedResultFieldName = 'GroupRefractiveIndex';
    requestedFieldFirstDim = 1;
    groupRefractiveIndexs = getRayTraceResultFieldForAllSurfaces( ...
