function plotWavefrontRetardanceMap(optSystem,surfIndex,wavelengthIndices,...
        fieldIndices,sampleGridSize,plotPanelHandle)
    % Displays the wavefront Retardance (in degree) map of the system
    % (or subsystem before a particular  surface in the system) over the
    % pupil area for a given field point and  wavelength.
    ...
    [polarizedRayTracerResult,pupilMeshGrid,outsidePupilIndices] = multipleRayTracer(optSystem,wavelengthIndices,...
        fieldIndices,sampleGridSize,sampleGridSize,samplingType,rayTraceOptionStruct,endSurface);
    ...
    % the wavefront diattenuation map in the subplots.
    for wavIndex = 1:nWav
        for fieldIndex = 1:nField
            ...
            retardanceP (~outsidePupilIndices) = computeProperRetardance(totalPMatrix,totalQMatrix);
            surf(subplotAxes,X, Y, retardanceP*180/pi,'EdgeColor', 'None','facecolor', 'interp');
            view(2);
            axis ('equal');
            axis ('off');
        end
    end
    set(gcf,'Name',['Wavefront Retardance Map at surface : ',num2str(surfIndex)]);
end
