function newHarmonicField = HarmonicField(Ex,Ey,sampDistX,sampDistY,wavelen,center,direction,domain)
    if nargin == 0
        N = 65;
        Ex = zeros(N,N);
        Ex(24:40,24:40) = 1;
    ...
    if ~mod(nPoints1,2)
        nPoints1 = nPoints1 + 1;
        Ex = [Ex;zeros(1,nPoints2)];
        Ey = [Ey;zeros(1,nPoints2)];
    end
    if ~mod(nPoints2,2)
        nPoints2 = nPoints2 + 1;
        Ex = [Ex,zeros(nPoints1,1)];
        Ey = [Ey,zeros(nPoints1,1)];
    end
    ...
    newHarmonicField.Domain = domain; % 1 for Spatial domain, and 2 for spatial frequency 
