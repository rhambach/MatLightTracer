function [ returnDataStruct] = BareGlass(returnFlag,coatingParameters,inputDataStruct)
    % BareGlass A user defined function for BareGlass. The function returns differnt
    % parameters when requested by the main program. It follows the common format
    ...
    if nargin < 1
        disp(['Error: The function BareGlass() needs atleast one argument',...
            'the return type.']);
    ...
    if nargin < 2
        if returnFlag == 2
            disp(['Error: The function BareGlass() needs atleast all three ',...
