function [retardanceTrans,retardanceRef, wavelengthVectorInUm] = ...
        plotCoatingRetardanceVsWavelength(coating,incAngleInDeg,...
    ...
    % plotCoatingRetardanceVsWavelength: Plot retardance related to the coating 
    ...
    %    primWavLenInUm,indexBefore,indexAfter,axesHandle,tableHandle,textHandle)
    % Outputs:
    %   [retardanceTrans,retardanceRef, wavelengthVectorInUm]
    ...
    wavelengthVectorInUm =  minWavelengthInUm:wavelengthStepInUm:maxWavelengthInUm;
    referenceWavLenInUm = primWavLenInUm;
    [ampRs,ampRp,powRs,powRp] = ...
            getReflectionCoefficients(coating,wavelengthVectorInUm,...
            incAngleInDeg*pi/180,indexBefore,indexAfter,referenceWavLenInUm); 
    [ampTs,ampTp,powTs,powTp] = ...
            getTransmissionCoefficients(coating,wavelengthVectorInUm,...
            incAngleInDeg*pi/180,indexBefore,indexAfter,referenceWavLenInUm); 
    ...
    plot(axesHandle,wavelengthVectorInUm,retardanceTrans,wavelengthVectorInUm,retardanceRef);
    ...
    title(axesHandle,[coating.Name,': Retardance Vs Wavelength'],'FontSize',12)            
    if nargin >= 9
        % Display tabular data on the tableHandle
        newTable1 = [wavelengthVectorInUm',retardanceTrans',retardanceRef'];
        newTable1 = mat2cell(newTable1,[ones(1,size(newTable1,1))],[ones(1,size(newTable1,2))]);
        columnName1 = {'Wavelength (um)','Transmission Retardance (deg)','Reflection Retardance (deg)'};
