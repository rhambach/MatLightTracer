function [ returnDataStruct] = GaussianDecompositionFFT1DRMSE(...
        returnFlag,meritFunctionParameters,inputDataStruct)
    %GaussianDecompositionFFT1DRMSE (Root mean square error) Defines a merit function for computing
    %    the rms error of an far field diffraction patthern of input data and fitted data.
    ...
    switch returnFlag
        ...
        case 2 % The merit function value evaluated for given input parameters
            ...
            % Compute the Far field diffraction pattern
            deltaInSpatial = 1; dimension = 1; zoomFactor = 10;
            givenAmpVectorFFT = abs((computeCZFFT(givenAmpVector,deltaInSpatial,dimension,zoomFactor)));
            totalAmpGaussFFT = abs((computeCZFFT(totalAmpGauss,deltaInSpatial,dimension,zoomFactor)));
            
            returnDataStruct.Value = sqrt(sum((givenAmpVectorFFT(:)-totalAmpGaussFFT(:)).^2)/nx); 
