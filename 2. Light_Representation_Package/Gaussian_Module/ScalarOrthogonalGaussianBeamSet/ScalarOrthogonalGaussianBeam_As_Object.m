classdef ScalarGaussianBeam
    % ScalarGaussianBeam class:
    %
    ...
    % <<<<<<<<<<<<<<<<<<<<< Main Code Section >>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    properties
        CentralRay          % 1 X N ScalarRay
        WaistRadiusInX      % 1 X N Double
        WaistRadiusInY      % 1 X N Double
        DistanceFromWaistInX   % 1 X N Double
        DistanceFromWaistInY   % 1 X N Double
        PeakAmplitude       % 1 X N Double
        LocalXDirection     % 3 X N Double
        LocalYDirection     % 3 X N Double
    ...
    methods
        function NewGaussianBeam = ScalarGaussianBeam(centralRay,...
            ...
            if nargin == 0
                ...
                NewGaussianBeam.WaistRadiusInX = 0.25; % Default value in lens units
    ...
    methods(Static)
        function newObj = InputGUI()
            newObj = ObjectInputDialog(MyHandle(ScalarGaussianBeam()));
        end
    end         
end
